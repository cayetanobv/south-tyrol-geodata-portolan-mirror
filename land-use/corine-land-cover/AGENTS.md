# AGENTS.md — Corine land cover — mirror of «Copertura del suolo CORINE», Provincia Autonoma di Bolzano
...
Source: WFS layer `p_bz-LandUse:CorineLandCover` of the Province's geoportal, mirrored to GeoParquet with spatial (Hilbert) ordering and a `bbox` column; a PMTiles archive and a MapLibre style (`styles/default.json`, derived from the provincial WMS style) accompany it. Geometry: Polygon; features: 2553.
...
Official record of the source dataset: https://data.civis.bz.it/dataset/corine-landcover

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('corine-land-cover.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('corine-land-cover.parquet')`.
