# AGENTS.md — School bus stops — mirror of «Trasporto alunni: Nodi di partenza», Provincia Autonoma di Bolzano
...
Official record of the source dataset: https://data.civis.bz.it/dataset/trasporto-alunni-nodi-di-partenza

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('school-bus-stops.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('school-bus-stops.parquet')`.
