# AGENTS.md — Intermunicipal cycleways — mirror of «Rete ciclabile sovracomunale», Provincia Autonoma di Bolzano
...
Official record of the source dataset: https://data.civis.bz.it/dataset/rete-ciclabile-sovracomunale

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('intermunicipal-cycleways.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('intermunicipal-cycleways.parquet')`.
