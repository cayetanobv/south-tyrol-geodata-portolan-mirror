# AGENTS.md — Public transport stops — mirror of «Fermate del trasporto pubblico», Provincia Autonoma di Bolzano
...
Source: WFS layer `p_bz-TransportNetwork:PublicTransport-Stops` of the Province's geoportal, mirrored to GeoParquet with spatial (Hilbert) ordering and a `bbox` column; a PMTiles archive and a MapLibre style (`styles/default.json`, derived from the provincial WMS style) accompany it. Geometry: Point; features: 7185.
...
Official record of the source dataset: https://data.civis.bz.it/dataset/fermate-del-trasporto-pubblico

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('public-transport-stops.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('public-transport-stops.parquet')`.
