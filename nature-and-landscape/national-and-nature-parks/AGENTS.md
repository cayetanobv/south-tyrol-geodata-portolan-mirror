# AGENTS.md — National and nature parks — mirror of «Parchi naturali e parco nazionale», Provincia Autonoma di Bolzano
...
Official record of the source dataset: https://data.civis.bz.it/dataset/parchi-naturali-e-parco-nazionale

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('national-and-nature-parks.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('national-and-nature-parks.parquet')`.
