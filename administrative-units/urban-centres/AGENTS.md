# AGENTS.md — Urban centres — mirror of «Centri edificati (LP 10/91)», Provincia Autonoma di Bolzano
...
Source: WFS layer `p_bz-AdministrativeUnits:Places-UrbanCenters` of the Province's geoportal, mirrored to GeoParquet with spatial (Hilbert) ordering and a `bbox` column; a PMTiles archive and a MapLibre style (`styles/default.json`, derived from the provincial WMS style) accompany it. Geometry: Point; features: 574.
...
Official record of the source dataset: https://data.civis.bz.it/dataset/centri-edificati-lp-10-91

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('urban-centres.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('urban-centres.parquet')`.
