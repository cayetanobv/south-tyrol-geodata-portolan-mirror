# AGENTS.md — Hazard zone plans coverage — mirror of «Piano delle Zone di Pericolo: Copertura», Provincia Autonoma di Bolzano
...
Official record of the source dataset: https://data.civis.bz.it/dataset/piani-delle-zone-di-pericolo-copertura

## Accessing the data

```sql
INSTALL spatial; LOAD spatial;
SELECT * EXCLUDE (geometry) FROM read_parquet('hazard-zone-plans-coverage.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('hazard-zone-plans-coverage.parquet')`.
