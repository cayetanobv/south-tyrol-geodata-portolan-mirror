# AGENTS.md — Civil protection plan status — mirror of «Stato dei Piani di Protezione Civile comunali», Provincia Autonoma di Bolzano

Agent-oriented guidance for this collection. Supplements `README.md` (people) and `collection.json` (software).

## Overview

Community mirror of the dataset «Stato dei Piani di Protezione Civile comunali» published by the Provincia Autonoma di Bolzano – Alto Adige / Autonome Provinz Bozen – Südtirol as open data (CC0 1.0). Extracted on 2026-09-03 from WFS layer p_bz-TerritorialPlans:CivilProtectionPlan-Status and converted to GeoParquet. This copy is NOT the official publication: the authoritative source is the Province's geoportal and open-data portal (https://data.civis.bz.it/dataset/piani-di-protezione-civile-comunali-ppcc). Published for the OGC Metadata Summit 2026 demo.

Source: WFS layer `p_bz-TerritorialPlans:CivilProtectionPlan-Status` of the Province's geoportal, mirrored to GeoParquet with spatial (Hilbert) ordering and a `bbox` column; a PMTiles archive and a MapLibre style (`styles/default.json`, derived from the provincial WMS style) accompany it. Geometry: Polygon; features: 116.
...
SELECT * EXCLUDE (geometry) FROM read_parquet('civil-protection-plan-status.parquet') LIMIT 5;
```

CRS is **OGC:CRS84 (WGS 84 lon/lat)**. Filter on the `bbox` struct column for spatial subsets (row-group pruning). Python: `geopandas.read_parquet('civil-protection-plan-status.parquet')`.
...
| `ISTAT_CODE` | int64 |  |
| `CONTRIBUTION_IT` | string |  |
| `CONTRIBUTION_DE` | string |  |
| `CONTRIBUTION_YEAR` | int64 |  |
| `STATUS_IT` | string |  |
| `STATUS_DE` | string |  |
| `VERSION` | double |  |
